...
public static class CalebUtils
{
    ...
    public static List<(Transform, float)> SortListByDistance(List<Collider> list, Transform origin)
    {
        List<(Transform, float)> returnList = new List<(Transform, float)>();

        foreach (var collider in list)
        {
            returnList.Add((collider.transform, Vector3.Distance(origin.position, collider.transform.position)));
        }
    ...
    public static List<(Transform, float, RaycastHit)> SortListByDistance(List<RaycastHit> list, Transform origin)
    {
        List<(Transform, float, RaycastHit)> returnList = new List<(Transform, float, RaycastHit)>();

        foreach (var hit in list)
        {
            returnList.Add((hit.transform, Vector3.Distance(origin.position, hit.transform.position), hit));
        }
